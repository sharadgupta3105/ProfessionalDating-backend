...
LinkedUp sends a **6-digit code** to the user's email on `POST /auth/login` when Resend is configured.
...
1. Resend → **Domains** → add `linkedup.app` (or your domain).
2. Add the DNS records Resend shows (SPF, DKIM).
3. After verified:

```env
OTP_FROM_EMAIL=LinkedUp <noreply@linkedup.app>
```
...
|------|--------|
| `RESEND_API_KEY` | `re_...` |
| `OTP_FROM_EMAIL` | `LinkedUp <noreply@yourdomain.com>` or test sender |
